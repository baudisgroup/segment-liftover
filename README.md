...
segmentLiftover is designed to process a large number of files in one run. 
...
### Chain files
A chain file is required by the UCSC LiftOver program to convert from one assemble to another, therefore, it is also **required** by segmentLiftover.

Common chain files for human are provider by segmentLiftover, please check [manual]() for details.
